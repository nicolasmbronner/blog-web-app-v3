...
4. **Gmail-style toasts** appear when deleting an article, when canceling article creation, when canceling article editing (same prototype 👆🏻), but also when trying to delete OR edit an article already being edited (it's protected during editing). This allows the user to cancel their last action (except for the protected article warning).
...
7. **Articles being edited are protected** and cannot be deleted or edited by another user during editing. A toast displays to indicate to the user that it's impossible to delete or edit the article during its editing by another user. Articles stay protected during the toast if a user cancels article edition. [See this discussion thread with Claude](https://claude.ai/chat/26cc7e36-a1b3-4479-ac00-6933c2bec52d).